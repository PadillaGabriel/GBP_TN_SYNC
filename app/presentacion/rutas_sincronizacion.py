...
def obtener_status_stock(db: Session = Depends(obtener_sesion_bd)) -> dict[str, object]:
...
async def ejecutar_prueba_parcial_gbp(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error en prueba GBP: {exc}"
        ) from exc
...
async def ejecutar_prueba_producto_gbp(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error en prueba producto GBP: {exc}"
        ) from exc
...
async def diagnosticar_descripcion_producto_gbp(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error diagnosticando descripción GBP: {exc}"
        ) from exc
...
async def importar_prueba_tienda_nube(
    ...
    - Solo usa productos con una decisión publicable vigente.
    """
