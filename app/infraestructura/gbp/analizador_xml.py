...
def reparar_mojibake(value: str | None) -> str | None:
    ...
    for encoding in ("latin1", "cp1252"):
        try:
            repaired = text.encode(encoding, errors="strict").decode(
                "utf-8", errors="strict"
            )
        except UnicodeError:
            continue
        ...
        if (
            repaired
            and repaired != text
            and repaired_marker_count < original_marker_count
        ):
            return repaired
...
def extract_node_full_text(node: ET.Element) -> str:
    ...
    def walk(current: ET.Element) -> None:
        ...
        if current is not node and tag in {
            "p",
            "div",
            "li",
            "ul",
...
def any_website_image(row: dict[str, str]) -> bool:
    ...
    return any(
        has_value(row.get(f"item_WebSite_url4Image{index}")) for index in range(1, 11)
    )
