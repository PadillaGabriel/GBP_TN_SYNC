...
class TiendaNubeCategoryService:
    ...
    def __init__(self, *, settings: Settings, audit_repo: SyncAuditRepository | None = None) -> None:
        self.settings = settings
        self.audit_repo = audit_repo
        self.client = TiendaNubeClient(
    ...
    async def normalizar_categorias_duplicadas(self, *, confirm: bool = False) -> dict[str, Any]:
        ...
        id_map, categorias_a_crear = await self._build_canonical_map(nodes, confirm=confirm)
        ...
        if confirm and id_map:
            ...
            for product in products:
                ...
                categorias_nuevas = self._dedupe_ids([
                    id_map.get(category_id, category_id) for category_id in categorias_actuales
                ])
        ...
        duplicate_ids = [dup_id for dup_id, canonical_id in id_map.items() if dup_id != canonical_id]
        if confirm and duplicate_ids:
            # Primero hijos, después padres.
            duplicate_ids_sorted = sorted(
                duplicate_ids,
                key=lambda category_id: id_to_node.get(category_id, _CategoryNode(category_id, "", "", None, 0)).depth,
                reverse=True,
            )
            for category_id in duplicate_ids_sorted:
                try:
                    ...
                    logger.exception("tn_category_delete_duplicate_failed", extra={"category_id": category_id})
                    errores_eliminacion.append({"category_id": category_id, "error": f"{type(exc).__name__}: {exc}"})
    ...
    def _build_nodes(cls, categories: list[dict[str, Any]]) -> list[_CategoryNode]:
        ...
        parent_by_id = {category_id: parent_id for category_id, _name, _norm, parent_id in raw_nodes}
        return [
            _CategoryNode(
                id=category_id,
                name=name,
    ...
    def _extract_product_category_ids(cls, product: dict[str, Any]) -> list[int]:
        ...
        if not isinstance(categories, list):
            return result
        for category in categories:
            category_id = cls._extract_id(category) if isinstance(category, dict) else cls._parse_int(category)
    ...
    def _category_es_name(category: dict[str, Any]) -> str:
        ...
        if isinstance(name, dict):
            return normalizar_texto_gbp(str(name.get("es") or name.get("pt") or name.get("en") or "")).strip()
