...
class GBPAuditService:
    ...
    def __init__(
        self, settings: ConfiguracionAplicacion, db: Session | None = None
    ) -> None:
        self.settings = settings
        self.db = db
        self.client = ClienteGBP(
    ...
    async def ejecutar_prueba_parcial(
        ...
        *,
        ...
        publicables = sum(
            1 for row in resultados if row.get("decision") == "PUBLICABLE_PARCIAL"
        )
        errores = sum(1 for row in resultados if row.get("error"))
        bloqueados = len(resultados) - publicables - errores

        if self.db is not None:
            RepositorioAuditoriaSincronizacion(self.db).registrar(
                sku=None,
                accion="GBP_TEST",
        ...
        *,
        ...
        try:
            ...
            if solo_no_auditados and self.db is not None:
                skus_auditados = RepositorioProductos(self.db).obtener_skus_auditados()
                candidatos_base = [
                    row
                    for row in candidatos
                    if str(row.get("item_code") or "").strip() not in skus_auditados
                ]
                resumen["candidatos_ya_auditados"] = len(candidatos) - len(
                    candidatos_base
                )
            ...
            for row in resultados_validos:
                ...
                if row.get("error"):
                    resumen["errores"] += 1
                elif es_decision_publicable(decision):
            ...
            resumen["ok"] = (
                resumen["errores"] == 0 and resumen["errores_persistencia"] == 0
            )
            ...
            if self.db is not None and guardar_en_db:
                try:
                    RepositorioAuditoriaSincronizacion(self.db).registrar(
                        sku=None,
                        accion="GBP_AUDIT_PRODUCTS_NEXT"
                        if resumen.get("modo_incremental")
                        else "GBP_AUDIT_PRODUCTS_RUN",
                        estado="ERROR_GLOBAL",
    ...
    def _persistir_resultados_auditoria(
        ...
        productos_repo = RepositorioProductos(self.db)  # type: ignore[arg-type]
        audit_repo = RepositorioAuditoriaSincronizacion(self.db)  # type: ignore[arg-type]
        ...
        for row in resultados:
            ...
            try:
                ...
                productos_repo.guardar_validacion(
                    producto_model.id, producto, resultado
                )
    ...
    def _registrar_resumen_auditoria(self, resumen: dict[str, Any]) -> None:
        ...
        try:
            ...
            RepositorioAuditoriaSincronizacion(self.db).registrar(  # type: ignore[arg-type]
                sku=None,
                accion="GBP_AUDIT_PRODUCTS_NEXT"
                if resumen.get("modo_incremental")
                else "GBP_AUDIT_PRODUCTS_RUN",
    ...
    async def ejecutar_prueba_producto(
        ...
        *,
        ...
        detalle = await self.client.obtener_producto_por_id(
            token, int(resolved_item_id)
        )
        imagenes = await self.client.obtener_imagenes_website_por_item_id(
            token, int(resolved_item_id)
        )
        ...
        if precio is not None:
            ...
            detalle["prli_id"] = precio.lista_precio_id or str(
                self.settings.online_price_list_id
            )
        ...
        if self.db is not None and guardar_en_db:
            producto_model = RepositorioProductos(self.db).guardar_producto(producto)
            RepositorioProductos(self.db).guardar_validacion(
                producto_model.id, producto, resultado
            )
            if producto.stock is not None:
                RepositorioProductos(self.db).guardar_stock(
                    producto_model.id, producto.stock
                )
            RepositorioAuditoriaSincronizacion(self.db).registrar(
                sku=producto.sku,
    ...
    async def diagnosticar_descripcion_producto(
        ...
        *,
        ...
        for key, value in detalle.items():
            ...
            if not any(
                token in key_lower
                for token in ("desc", "detail", "web", "html", "memo", "note")
            ):
                continue
            campos_relevantes.append(
                {
                    "campo": key,
                    "largo": len(text),
                    "es_candidato_descripcion_web": self.normalizer._is_web_description_key(
                        str(key)
                    ),
                    "preview": text[:500],
                }
        ...
        return normalizar_objeto_gbp(
            {
                ...
                "descripcion_seleccionada_preview": (producto.descripcion_web or "")[
                    :1000
                ],
    ...
    async def _procesar_candidato_completo(
        ...
        async with semaphore:
            ...
            try:
                ...
                logger.exception(
                    "gbp_audit_product_error", extra={"item_id": item_id, "sku": sku}
                )
                return normalizar_objeto_gbp(
                    {
                        "sku": sku,
