...
from app.aplicacion.servicios.servicio_auditoria_gbp import GBPAuditService
from app.aplicacion.servicios.servicio_sincronizacion_stock import StockSyncService
from app.aplicacion.servicios.servicio_categorias_tienda_nube import (
    TiendaNubeCategoryService,
)
...
from app.dominio.decisiones_producto import es_decision_publicable
...
def _update_job(
    ...
    *,
    ...
    with SessionLocal() as db:
        RepositorioTrabajosSincronizacion(db).actualizar(
            job_id,
            estado=estado,
            progreso=progreso,
            finalizar=finalizar,
...
def _cancel_requested(job_id: int) -> bool:
    ...
    with SessionLocal() as db:
        return RepositorioTrabajosSincronizacion(db).cancelacion_solicitada(job_id)
    ...
    *,
    ...
    try:
        ...
        while True:
            ...
            total_errores += int(result.get("errores") or 0) + int(
                result.get("errores_persistencia") or 0
            )
        ...
        with SessionLocal() as db:
            RepositorioAuditoriaSincronizacion(db).registrar(
                sku=None,
        ...
        _update_job(
            ...
            progreso={
                ...
                "porcentaje": _percent(
                    total_procesados,
                    max(total_procesados + int(ultimo_pendiente or 0), 1),
                ),
            },
        )
...
async def ejecutar_job_importar_todo(
    *,
    ...
    try:
        with SessionLocal() as db:
            resumen = RepositorioProductos(db).resumen_operativo_panel()
        ...
        while True:
            ...
            with SessionLocal() as db:
                pendientes = int(
                    RepositorioProductos(db)
                    .resumen_operativo_panel()
                    .get("publicables_pendientes_importar")
                    or 0
                )
            if pendientes <= 0:
                break
            ...
            with SessionLocal() as db:
                ...
                result = await service.importar_prueba_tienda_nube(
                    limit=batch_limit, confirm=True
                )
            ...
            if procesados <= 0 or (
                seleccionados > 0 and int(result.get("errores") or 0) >= seleccionados
            ):
            ...
            _update_job(
                ...
                progreso={
                    ...
                    "porcentaje": _percent(
                        total_procesados,
                        max(total_inicial, total_procesados + pendientes),
                    ),
                    "ultima_tanda": {
                ...
                if sin_avance_consecutivo >= 1 and pendientes > 0:
                    ...
                    _update_job(
                        ...
                        progreso={
                            ...
                            "pendientes": pendientes,
                            "porcentaje": _percent(
                                total_procesados, max(total_inicial, 1)
                            ),
                        },
                    )
                    return
    ...
    *,
    ...
    try:
        ...
        _update_job(
            ...
            estado="EN_PROCESO",
            progreso={
                "mensaje": "Auditando próximos productos.",
                "porcentaje": 5,
                "batch_limit": batch_limit,
            },
            finalizar=False,
        ...
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(job_id, iniciar=True)
            service = GBPAuditService(settings=settings, db=db)
            result = await service.ejecutar_auditoria_productos(
                limit=batch_limit,
                concurrency=concurrency,
            ...
            RepositorioAuditoriaSincronizacion(db).registrar(
                ...
                estado="OK"
                if int(result.get("errores") or 0) == 0
                else "OK_CON_ERRORES",
                mensaje=(
        ...
        _update_job(
            ...
            progreso={
                ...
                "pendientes_por_auditar": int(
                    result.get("candidatos_pendientes_auditar") or 0
                ),
        ...
        _update_job(
            ...
            finalizar=True,
            error_codigo=type(exc).__name__,
            progreso={"mensaje": str(exc), "porcentaje": 100},
        )
    ...
    try:
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(
                ...
                iniciar=True,
                progreso={
                    "mensaje": f"Consultando e importando SKU {sku}.",
                    "porcentaje": 10,
                    "sku": sku,
                    "forzar": forzar,
                },
            ...
            result = await service.importar_producto_manual_tienda_nube(
                sku=sku, confirm=True, forzar=forzar
            )
        ...
        logger.exception(
            "job_importar_sku_tienda_nube_http_error",
            extra={"job_id": job_id, "sku": sku},
        )
        _update_job(
            job_id,
            estado="ERROR",
        ...
        logger.exception(
            "job_importar_sku_failed", extra={"job_id": job_id, "sku": sku}
        )
        _update_job(
            job_id,
            estado="ERROR",
            finalizar=True,
            error_codigo=type(exc).__name__,
            progreso={"mensaje": str(exc), "porcentaje": 100, "sku": sku},
        )
    ...
    *,
    ...
    try:
        ...
        _update_job(
            ...
            progreso={
                **result,
                "mensaje": "Sincronización de stock finalizada.",
                "porcentaje": 100,
            },
        )
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(
                ...
                iniciar=True,
                progreso={
                    "mensaje": f"Sincronizando stock SKU {sku}.",
                    "porcentaje": 10,
                    "sku": sku,
                },
            )
        ...
        _update_job(
            ...
            progreso={
                **result,
                "mensaje": f"Stock SKU {sku}: {result.get('estado', '-')}",
                "porcentaje": 100,
            },
        )
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(
                ...
                iniciar=True,
                progreso={
                    "mensaje": "Normalizando categorías duplicadas.",
                    "porcentaje": 10,
                },
            )
        ...
        errores = len(result.get("errores_productos", [])) + len(
            result.get("errores_eliminacion", [])
        )
        _update_job(
            job_id,
            estado="FINALIZADO" if errores == 0 else "FINALIZADO_CON_ERRORES",
            finalizar=True,
            progreso={
                **result,
                "mensaje": "Normalización de categorías finalizada.",
                "errores": errores,
                "porcentaje": 100,
            },
        )
...
async def ejecutar_job_reauditar_decision(
    *,
    ...
    try:
        ...
        for sku in skus:
            ...
            try:
                with SessionLocal() as db:
                    service = TiendaNubeImportService(settings=settings, db=db)
                    result = await service.importar_producto_manual_tienda_nube(
                        sku=sku, confirm=False, forzar=False
                    )
                    RepositorioAuditoriaSincronizacion(db).registrar(
                        sku=sku,
                ...
                total += 1
                if es_decision_publicable(result.get("decision")):
                    publicables += 1
                else:
                    siguen_bloqueados += 1
                if len(detalle) < 30:
                ...
                if len(detalle) < 30:
                    detalle.append(
                        {"sku": sku, "error": f"{type(exc).__name__}: {exc}"}
                    )
            _update_job(
                job_id,
                estado="EN_PROCESO",
        ...
        logger.exception(
            "job_reauditar_decision_failed",
            extra={"job_id": job_id, "decision": decision},
        )
        _update_job(
            job_id,
            estado="ERROR",
    ...
    try:
        ...
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(
                ...
                iniciar=True,
                progreso={
                    "mensaje": "Reconciliando mapeos con Tienda Nube.",
                    "porcentaje": 5,
                    "limit": limit,
                },
            )
        ...
        _update_job(
            ...
            progreso={
                **result,
                "mensaje": "Reconciliación finalizada.",
                "porcentaje": 100,
            },
        )
    except Exception as exc:  # noqa: BLE001
    ...
    try:
        ...
        with SessionLocal() as db:
            RepositorioTrabajosSincronizacion(db).actualizar(
                ...
                iniciar=True,
                progreso={
                    "mensaje": "Marcando mapeos locales como eliminado_externo.",
                    "porcentaje": 20,
                },
            )
            service = TiendaNubeImportService(settings=settings, db=db)
        ...
        _update_job(
            ...
            progreso={
                **result,
                "mensaje": "Reset de mapeos locales finalizado.",
                "porcentaje": 100,
            },
        )
    except Exception as exc:  # noqa: BLE001
